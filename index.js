...
const ZILE_PRODUS_NOU = 17 * 365;

function isProductNew(dataLansare) {
    if (!dataLansare) return false;
    const dataCurenta = new Date();
    const dataLansareObj = new Date(dataLansare);
    const diferentaMs = dataCurenta - dataLansareObj;
    const diferentaZile = diferentaMs / (1000 * 60 * 60 * 24);
    return diferentaZile <= ZILE_PRODUS_NOU;
}

app.get(["/", "/index", "/home"], function (req, res) {
    client.query("SELECT * FROM albume ORDER BY data_lansare DESC LIMIT 6", function (err, rezProdNoi) {
        if (err) {
            console.error("Eroare la produse noi:", err);
            rezProdNoi = { rows: [] };
        }

        if (rezProdNoi && rezProdNoi.rows) {
            rezProdNoi.rows.forEach(prod => {
                prod.isNou = isProductNew(prod.lansare);
            });
        }
...
app.get("/produse", function (req, res) {
    ...
    //modif din req.query.tip
    if (req.query.categ_muzicala && req.query.categ_muzicala !== 'toate') {
        clauzaWhere = ` WHERE categ_muzicala='${req.query.categ_muzicala}'`;
    }

    client.query(`SELECT * FROM albume ${clauzaWhere}`, function (err, rez) {
        if (err) {
            console.error("Eroare la albume:", err);
            return afisareEroare(res, 100);
        }

        // bonus 1 etapa 6 
        client.query("SELECT * FROM unnest(enum_range(NULL::categ_muzicale))", async function (err, rezOptiuni) {
            if (err) {
            ...
            try {
                ...
                rez.rows.forEach(prod => {
                    prod.isNou = isProductNew(prod.data_lansare);
                });

                let pretMinPeCategorie = {};
                
                for (let prod of rez.rows) {
                    let categ = prod.categ_muzicala;
                    
                    if (!pretMinPeCategorie[categ]) {
                        pretMinPeCategorie[categ] = prod.pret;
                    } else if (prod.pret < pretMinPeCategorie[categ]) {
                        pretMinPeCategorie[categ] = prod.pret;
                    }
                }

                rez.rows.forEach(prod => {
                    prod.isCelMaiIeftin = (prod.pret === pretMinPeCategorie[prod.categ_muzicala]);
                });

                let rezPiese = await client.query("SELECT MIN(numar_piese) AS min, MAX(numar_piese) AS max FROM albume");
                let pieseMin = rezPiese.rows[0].min;
                let pieseMax = rezPiese.rows[0].max;

                let rezCulori = await client.query("SELECT DISTINCT culoare FROM albume WHERE culoare IS NOT NULL");
                let culori = [];
                for (let i = 0; i < rezCulori.rows.length; i++) {
                    culori.push(rezCulori.rows[i].culoare);
                }

                let rezFormat = await client.query("SELECT DISTINCT format FROM albume WHERE format IS NOT NULL");
                let formate = [];
                for (let i = 0; i < rezFormat.rows.length; i++) {
                    formate.push(rezFormat.rows[i].format);
                }

                let rezPret = await client.query("SELECT MIN(pret) AS min, MAX(pret) AS max FROM albume");
                let pMin = parseFloat(rezPret.rows[0].min);
                let pMax = parseFloat(rezPret.rows[0].max);
                let step = Math.ceil((pMax - pMin) / 3);

                let intervalePret = [
                    { id: "i_rad1", label: "Ieftin", val: `${pMin}:${pMin + step}` },
                    { id: "i_rad2", label: "Mediu", val: `${pMin + step + 1}:${pMin + 2 * step}` },
                    { id: "i_rad3", label: "Scump", val: `${pMin + 2 * step + 1}:1000000` }
                ];

                let numePlaceholder = "Master of Puppets";
                if (rez.rows.length > 0) {
                    let indexRandom = Math.floor(Math.random() * rez.rows.length);
                    numePlaceholder = rez.rows[indexRandom].nume;
                }
                ...
                console.error("Eroare la interogarile pentru filtre:", err);
...
const ANOTIMPURI = ["iarna", "primavara", "vara", "toamna"]
...
function currentSeason(){
    luna = currentMonthIndex();
    if(luna == 6 || luna == 7 || luna == 8 )return ANOTIMPURI[2];
    // intuitiv...
...
app.get("/produs/:id", function (req, res) {
    ...
    client.query("SELECT * FROM albume WHERE id = $1", [id], function (err, rez) {
        if (err) {
